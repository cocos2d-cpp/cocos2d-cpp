...
void TileDemo::onTouchesMoved(const std::vector<Touch*>& touches, Event *)
{
...
TileMapTest::TileMapTest()
{
    ...
    CCLOG("ContentSize: %f, %f", map->getContentSize().width, map->getContentSize().height);
...
TMXOrthoFlipTest::TMXOrthoFlipTest()
{
    ...
    log("ContentSize: %f, %f", map->getContentSize().width, map->getContentSize().height);
...
TMXOrthoFromXMLTest::TMXOrthoFromXMLTest()
{
    ...
    std::string filename = resources + "/orthogonal-test1.tmx";
    std::string fullPath = fileUtils->fullPathForFilename(filename);
    std::string str = fileUtils->getStringFromFile(fullPath);
